...
function FreeNavbar({yes}) {
  ...
  return (
    <div className=" border-b-[1px]">
      ...
      <div className="sm:pl-10 sm:pr-10 pl-4 pr-4 flex flex-row bg-[#161515] justify-between w-full">
      <div className="flex flex-row">
        ...
        <Link to="/freehome" className={`pl-6 pr-6 pt-4 pb-4 text-white font-semibold cursor-pointer border-b-[2px] border-transparent hover:border-blue-600 ${yes==='Y'?'hidden':'lg:block hidden'}`}>
          Dashboard
        </Link>
        <Link to="/allfreelancers" className={`pl-6 pr-6 pt-4 pb-4 text-white font-semibold cursor-pointer border-b-[2px] border-transparent hover:border-blue-600 ${yes==='Y'?'hidden':'lg:block hidden'}`}>
          Find Freelancers
        </Link>
        <Link to='/about' className={`pl-6 pr-6 pt-4 pb-4 text-white font-semibold cursor-pointer border-b-[2px] border-transparent hover:border-blue-600 ${yes==='Y'?'hidden':'lg:block hidden'}`}>
          About
        </Link>
      </div>
      <div className="flex flex-row">
        <Link to="/home" className={` lg:flex lg:flex-col hidden justify-center hover:text-white mt-2 mb-2 cursor-pointer bg-white text-black hover:bg-blue-600 sm:pl-4 sm:pr-4 pl-3 pr-3 rounded-md ${yes==='Y'?'hidden':''} sm:mr-6 mr-1`}>
          <h2 className="sm:text-base text-sm font-semibold">Funds</h2>
        </Link>
        ...
        <Link to="/alljobs" className={` lg:flex lg:flex-col hidden justify-center hover:text-white mt-2 mb-2 cursor-pointer bg-white text-black hover:bg-blue-600 sm:pl-4 sm:pr-4 pl-3 pr-3 rounded-md ${yes==='Y'?'hidden':''}`}>
        ...
        <div className="ml-2 flex flex-col justify-center" >
          <img onClick={changeNav} className="h-10 w-10 cursor-pointer rounded-full" src={ pic2 || myFreelancer?.image} alt="" />
        </div>
      </div>
      </div>
      <div className={`lg:hidden shadow-lg rounded-b bg-gray-300 border-b-[2px] border-l-[2px] border-r-[2px] border-gray-600 absolute w-screen z-30 pt-2 pb-2 sm:pl-12 sm:pr-12 pl-6 pr-6 ${ys===0?"hidden":""}`}>
       ...
       <div>
       <Link to="/freehome" className={`pt-1 pb-1 mt-1 sm:text-base text-sm font-semibold cursor-pointer ${yes==='Y'?'hidden':''}`}>
          Dashboard
        </Link>
        </div> 
        <div>
      ...
      <div className={`bg-white shadow-lg absolute sm:right-14 right-9 z-30 top-14 mt-[1px] p-4 ${nav===0?"hidden":""} text-black`}>
